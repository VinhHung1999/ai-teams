---
name: tmux-team-creator-mcp
description: This skill should be used when users want to create a multi-agent tmux team for their project. It provides battle-tested templates for setting up autonomous AI agent teams that collaborate via tmux, with MCP-based board management for the scrum-team template. Use this skill when users ask to create AI teams, set up multi-agent workflows, or build autonomous coding teams. The skill includes 4 sample team templates (2 software development + 2 research) that can be customized for any domain.
---
...
- MCP-based board management (backlog, sprint board, task status via MCP tools instead of markdown files)
...
│   # NOTE: Backlog/board/task management uses MCP tools:
│   #   list_backlog, create_backlog_item, add_item_to_sprint,
│   #   get_board, get_my_tasks, update_task_status, add_task_note, list_sprints
│
...
| Backlog | MCP tools (list_backlog, get_board, etc.) | WHITEBOARD |
