...
**RIGHT:** Boss says something → Add to backlog (`create_backlog_item`) → Prioritize → Plan for next sprint

**Boss should NEVER have to remind PO to add things to backlog.** If Boss says something, PO captures it via `create_backlog_item` automatically, then prioritizes for future sprints.

### Backlog Ownership

**PO owns PRODUCT BACKLOG. SM owns SPRINT BOARD.**

- **PO uses:** `create_backlog_item` MCP tool (directly, don't delegate to SM)
- **SM uses:** `add_item_to_sprint` MCP tool (after PO defines sprint scope)

**WRONG:** PO tells SM to add items to product backlog
**RIGHT:** PO adds items via `create_backlog_item` directly, then tells SM what's in next sprint
...
2. **Review backlog** - Use `list_backlog` MCP tool for next items
3. **Prioritize autonomously** - Select items for next sprint
4. **Create sprint goal** - Tell SM to add items to sprint via `add_item_to_sprint`
...
3. SM adds items to sprint via `add_item_to_sprint` MCP tool
...
1. Read: `workflow.md`
2. Use `get_board` MCP tool for current status
3. Use `list_backlog` MCP tool to review the product backlog
