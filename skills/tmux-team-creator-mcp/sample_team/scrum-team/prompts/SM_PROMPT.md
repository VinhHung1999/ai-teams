...
| Delete sprint | `delete_sprint` MCP tool |
...
```bash
# Correct
tm-send TL "SM [HH:mm]: Sprint Planning in 5 min. Check the board."
...
5. Add items to sprint via `add_item_to_sprint` MCP tool
...
- Board status via `get_board` MCP tool
