...
export default function ProjectPage({
  ...
  /* ─── Custom collision: pointerWithin first (zones), fallback closestCorners (columns) ─── */
  const customCollision: CollisionDetection = useCallback((args) => {
    // First check if pointer is within a droppable (good for zones)
    const pointerCollisions = pointerWithin(args);
    if (pointerCollisions.length > 0) return pointerCollisions;
    // Fallback to closestCorners for column-level sorting
  ...
  const handleDragStart = (event: DragStartEvent) => {
    ...
    if (data?.type === "backlog") {
      ...
      // Fallback: check if it's in active sprint columns
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    // Only handle reordering within active sprint columns for board items
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (data?.type === "backlog") {
      ...
      // Drop on another backlog card or on the backlog zone: no-op
      setIsDragging(false);
      return;
    }
    ...
    if (data?.type === "task") {
      ...
      // Dropped on backlog zone → remove from sprint
      ...
      // Dropped on a different planning sprint zone → move between sprints
      ...
      // Moved within active sprint columns
      if (activeBoard) {
        const activeCol = findActiveSprintColumn(active.id);
